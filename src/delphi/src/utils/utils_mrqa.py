...
from ..huggingface.pytorch_transformers.tokenization_bert import BasicTokenizer
...
def read_mrqa_datasets(query_input, sample_size, adversarial_sample_size = 0):
    # Read list of or single dirs/files
    return read_mrqa_dataset_mapper(query_input, sample_size, adversarial_sample_size)


def read_mrqa_dataset_mapper(query, sample_size, adversarial_sample_size = 0):
    examples = read_mrqa_dataset(query, sample_size, adversarial_sample_size)

    return {"examples": examples}


def read_mrqa_dataset(entry, sample_size=None, adversarial_sample_size = 0):
    """Read an MRQA json file into a list of MRQAExamples."""
    examples = []
    ...
    for qa in entry["qas"]:
        ...
        if adversarial_sample_size > 0:
            adversarial_example = construct_adversarial_example(example)
            if adversarial_example:
                adversarial_examples.append(adversarial_example)

    random.shuffle(examples)
    selected_examples = examples[0:sample_size]
    if adversarial_sample_size > 0:
        random.shuffle(adversarial_examples)
        selected_adversarial_examples = adversarial_examples[0:adversarial_sample_size]

        selected_examples += selected_adversarial_examples
        random.shuffle(selected_examples)

    return selected_examples


def construct_adversarial_example(original_example: MRQAExample):
    def select_question_token(question_tokens):
        random.shuffle(question_tokens)
        for token in question_tokens:
            # word token of token is more than just one character
            if len(token) > 1:
                return token

    def add_question_token_to_context(token, doc_tokens, start_token_pos, end_token_pos):
        # at most try 3 times
        insert_at = start_token_pos
        while insert_at >= start_token_pos and insert_at <= end_token_pos:
            insert_at = random.randrange(1, len(doc_tokens))

        assert insert_at != -1

        doc_tokens.insert(insert_at, token)

        off_set = 0
        if insert_at < start_token_pos:
            start_token_pos += 1
            end_token_pos += 1
            off_set = len(token)+1

        return doc_tokens, start_token_pos, end_token_pos, off_set

    adv_example = copy.deepcopy(original_example)
    adv_example.qas_id = str(uuid.uuid4()).replace("-", "")

    question_token = select_question_token(adv_example.question_tokens)
    if question_token is None:
        return None

    detected_answer = adv_example.detected_answers[0]
    original_token_start_pos = detected_answer["token_spans"][0][0]
    original_token_end_pos = detected_answer["token_spans"][0][1]
    adv_example.doc_tokens, start_token_pos, end_token_pos, off_set = add_question_token_to_context(question_token,
                                                                                               adv_example.doc_tokens,
                                                                                               original_token_start_pos,
                                                                                               original_token_end_pos)

    if original_token_start_pos != start_token_pos:
        adv_example.detected_answers[0]["token_spans"][0][0] = start_token_pos
        adv_example.detected_answers[0]["token_spans"][0][1] = end_token_pos
        adv_example.detected_answers[0]["char_spans"][0][0] = adv_example.detected_answers[0]["char_spans"][0][0] + off_set
        adv_example.detected_answers[0]["char_spans"][0][1] = adv_example.detected_answers[0]["char_spans"][0][0] + off_set
...
def _improve_answer_span(doc_tokens, input_start, input_end, tokenizer, orig_answer_text):
    ...
    tok_answer_text = " ".join(tokenizer.mrqa_tokenize_text(orig_answer_text))
...
def split_examples_into_segments(
    *,
    all_examples,
    num_example_split,
    ...
    assert (
        len(all_examples) >= num_example_split
    ), f"num_example_split cannot be less than number of examples ({len(all_examples)})"
    assert (
        num_example_split - (len(all_examples) % num_example_split)
        <= len(all_examples) // num_example_split
    ), "Must be able to evenly distribute our examples across splits"

    segment_feature_lens = []
    feature_segments = []
    for i in range(num_example_split):
        example_segment = all_examples[i * examples_per_segment : (i + 1) * examples_per_segment]
        assert len(example_segment) > 0
        if len(example_segment) > 1:
            assert (
                len(example_segment) >= batch_size
            ), "Number of training examples should be greater than batch size"

        train_feature_segment = mrqa_convert_examples_to_features(
            examples=example_segment,
            tokenizer=tokenizer,
        ...
        segment_feature_lens.append(len(train_feature_segment))
        feature_segments.append(train_feature_segment)

    return feature_segments, segment_feature_lens
