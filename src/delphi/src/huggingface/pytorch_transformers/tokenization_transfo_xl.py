...
class TransfoXLTokenizer(PreTrainedTokenizer):
    ...
    def build_vocab(self):
        if self.vocab_file:
            ...
            logger.info(
                "building vocab with min_freq={}, max_size={}".format(self.min_freq, self.max_size)
            )
            self.idx2sym = []
            self.sym2idx = OrderedDict()
            ...
            logger.info(
                "final vocab size {} from {} unique tokens".format(len(self), len(self.counter))
            )
...
class TransfoXLCorpus(object):
    ...
    def from_pretrained(cls, pretrained_model_name_or_path, cache_dir=None, *inputs, **kwargs):
        ...
        if resolved_corpus_file == corpus_file:
            ...
            logger.info(
                "loading corpus file {} from cache at {}".format(corpus_file, resolved_corpus_file)
            )
