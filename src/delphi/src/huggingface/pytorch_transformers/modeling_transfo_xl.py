...
class DecoderLayer(nn.Module):
    ...
    def forward(self, dec_inp, dec_attn_mask=None, mems=None, head_mask=None):
        ...
        attn_outputs = self.dec_attn(
            dec_inp, attn_mask=dec_attn_mask, mems=mems, head_mask=head_mask
        )
...
class RelLearnableDecoderLayer(nn.Module):
    ...
    def forward(
        self, dec_inp, r_emb, r_w_bias, r_bias, dec_attn_mask=None, mems=None, head_mask=None
    ):
...
class RelPartialLearnableDecoderLayer(nn.Module):
    ...
    def forward(self, dec_inp, r, dec_attn_mask=None, mems=None, head_mask=None):
        ...
        attn_outputs = self.dec_attn(
            dec_inp, r, attn_mask=dec_attn_mask, mems=mems, head_mask=head_mask
        )
        ff_output = self.pos_ff(attn_outputs[0])
...
class TransfoXLModel(TransfoXLPreTrainedModel):
    ...
    def __init__(self, config):
        ...
        self.word_emb = AdaptiveEmbedding(
            config.n_token, config.d_embed, config.d_model, config.cutoffs, div_val=config.div_val
        )
    ...
    def _forward(self, dec_inp, mems=None, head_mask=None):
        ...
        if self.same_length:
            ...
            dec_attn_mask = torch.triu(word_emb.new_ones(qlen, klen), diagonal=1 + mlen).byte()[
                :, :, None
            ]
        ...
        if self.attn_type == 0:  # default
            ...
            for i, layer in enumerate(self.layers):
                ...
                if mems_i is not None and i == 0:
                    mems_i += pos_emb[:mlen]
                layer_outputs = layer(
                    core_out, dec_attn_mask=dec_attn_mask, mems=mems_i, head_mask=head_mask[i]
                )
                core_out = layer_outputs[0]
                if self.output_attentions:
                    attentions.append(layer_outputs[1])
...
class TransfoXLLMHeadModel(TransfoXLPreTrainedModel):
    ...
    def forward(self, input_ids, labels=None, mems=None, head_mask=None):
        ...
        if self.sample_softmax > 0 and self.training:
            ...
            logit = sample_logits(
                self.transformer.word_emb, self.out_layer.bias, labels, pred_hid, self.sampler
            )
