...
def main():
    ...
    if (len(sys.argv) < 4 or len(sys.argv) > 6) or sys.argv[1] not in [
        "bert",
        "gpt",
        "transfo_xl",
        "gpt2",
        ...
        if sys.argv[1] == "bert":
            ...
            if len(sys.argv) != 5:
                ...
                print(
                    "Should be used as `pytorch_transformers bert TF_CHECKPOINT TF_CONFIG PYTORCH_DUMP_OUTPUT`"
                )
            else:
            ...
            if len(sys.argv) < 4 or len(sys.argv) > 5:
                ...
                convert_openai_checkpoint_to_pytorch(
                    OPENAI_GPT_CHECKPOINT_FOLDER_PATH, OPENAI_GPT_CONFIG, PYTORCH_DUMP_OUTPUT
                )
            ...
            if len(sys.argv) < 4 or len(sys.argv) > 5:
                # pylint: disable=line-too-long
                print(
                    "Should be used as `pytorch_transformers transfo_xl TF_CHECKPOINT/TF_DATASET_FILE PYTORCH_DUMP_OUTPUT [TF_CONFIG]`"
                )
            else:
            ...
            if len(sys.argv) < 4 or len(sys.argv) > 5:
                # pylint: disable=line-too-long
                print(
                    "Should be used as `pytorch_transformers gpt2 TF_CHECKPOINT PYTORCH_DUMP_OUTPUT [TF_CONFIG]`"
                )
            else:
                TF_CHECKPOINT = sys.argv[2]
                PYTORCH_DUMP_OUTPUT = sys.argv[3]
            ...
            if len(sys.argv) < 5 or len(sys.argv) > 6:
                # pylint: disable=line-too-long
                print(
                    "Should be used as `pytorch_transformers xlnet TF_CHECKPOINT TF_CONFIG PYTORCH_DUMP_OUTPUT [FINETUNING_TASK_NAME]`"
                )
            else:
                TF_CHECKPOINT = sys.argv[2]
                ...
                convert_xlnet_checkpoint_to_pytorch(
                    TF_CHECKPOINT, TF_CONFIG, PYTORCH_DUMP_OUTPUT, FINETUNING_TASK
                )
        elif sys.argv[1] == "xlm":
