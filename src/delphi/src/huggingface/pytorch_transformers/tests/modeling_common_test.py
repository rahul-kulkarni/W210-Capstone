                ...
                for name, param in model.named_parameters():
                    if param.requires_grad:
                        self.assertIn(
                            ...
                            [0.0, 1.0],
                            msg="Parameter {} of model {} seems not properly initialized".format(
                                name, model_class
                            ),
                        )
        ...
        def test_headmasking(self):
            ...
            for model_class in self.all_model_classes:
                ...
                head_mask = torch.ones(
                    self.model_tester.num_hidden_layers, self.model_tester.num_attention_heads
                )
                head_mask[0, 0] = 0
                head_mask[-1, :-1] = 0
    ...
    class GPTModelTester(CommonModelTester):
        ...
        def prepare_config_and_inputs(self):
            ...
            input_ids = ids_tensor(
                [self.batch_size, self.n_choices, self.seq_length], total_num_tokens
            )

            position_ids = None
            if self.use_position_ids:
                position_ids = ids_tensor(
                    [self.batch_size, self.n_choices, self.seq_length], self.n_positions
                )
            ...
            if self.use_labels:
                ...
                lm_labels = ids_tensor(
                    [self.batch_size, self.n_choices, self.seq_length], self.num_labels
                )
