...
class BertModelTest(CommonTestCases.CommonModelTester):
    ...
    class BertModelTester(object):
        ...
        def create_and_check_bert_model(
            ...
            self.parent.assertListEqual(
                list(result["pooled_output"].size()), [self.batch_size, self.hidden_size]
            )
        ...
        def create_and_check_bert_for_next_sequence_prediction(
            ...
            loss, seq_relationship_score = model(
                input_ids, token_type_ids, input_mask, sequence_labels
            )
            result = {"loss": loss, "seq_relationship_score": seq_relationship_score}
            self.parent.assertListEqual(
                list(result["seq_relationship_score"].size()), [self.batch_size, 2]
            )
            self.check_loss_output(result)
        ...
        def create_and_check_bert_for_pretraining(
            ...
            loss, prediction_scores, seq_relationship_score = model(
                input_ids, token_type_ids, input_mask, token_labels, sequence_labels
            )
            result = {
                "loss": loss,
        ...
        def create_and_check_bert_for_question_answering(
            ...
            loss, start_logits, end_logits = model(
                input_ids, token_type_ids, input_mask, sequence_labels, sequence_labels
            )
            result = {"loss": loss, "start_logits": start_logits, "end_logits": end_logits}
            self.parent.assertListEqual(
                list(result["start_logits"].size()), [self.batch_size, self.seq_length]
            )
            self.parent.assertListEqual(
                list(result["end_logits"].size()), [self.batch_size, self.seq_length]
            )
            self.check_loss_output(result)
        ...
        def create_and_check_bert_for_multiple_choice(
            ...
            result = {"loss": loss, "logits": logits}
            self.parent.assertListEqual(
                list(result["logits"].size()), [self.batch_size, self.num_choices]
            )
            self.check_loss_output(result)
