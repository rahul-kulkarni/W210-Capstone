...
def load_tf_weights_in_openai_gpt(model, config, openai_checkpoint_folder_path):
    ...
    names = json.load(
        open(openai_checkpoint_folder_path + "/parameters_names.json", "r", encoding="utf-8")
    )
    shapes = json.load(
        open(openai_checkpoint_folder_path + "/params_shapes.json", "r", encoding="utf-8")
    )
    offsets = np.cumsum([np.prod(shape) for shape in shapes])
    init_params = [
        np.load(openai_checkpoint_folder_path + "/params_{}.npy".format(n)) for n in range(10)
    ]
...
class OpenAIGPTLMHeadModel(OpenAIGPTPreTrainedModel):
    ...
    def forward(
        self, input_ids, position_ids=None, token_type_ids=None, labels=None, head_mask=None
    ):
