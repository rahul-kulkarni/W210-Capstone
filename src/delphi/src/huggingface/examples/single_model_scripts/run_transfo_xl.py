...
def main():
    ...
    parser.add_argument(
        "--model_name", type=str, default="transfo-xl-wt103", help="pretrained model name"
    )
    parser.add_argument(
        "--split",
        type=str,
    ...
    parser.add_argument(
        "--mem_len", type=int, default=1600, help="length of the retained previous heads"
    )
    parser.add_argument(
        "--clamp_len", type=int, default=1000, help="max positional embedding index"
    )
    parser.add_argument(
        "--no_cuda", action="store_true", help="Do not use CUDA even though CUA is available"
    )
    ...
    def evaluate(eval_iter):
        ...
        logger.info(
            "Time : {:.2f}s, {:.2f}ms/segment".format(total_time, 1000 * total_time / (idx + 1))
        )
