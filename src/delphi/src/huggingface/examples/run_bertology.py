...
def compute_heads_importance(
    ...
    np.save(
        os.path.join(args.output_dir, "head_importance.npy"), head_importance.detach().cpu().numpy()
    )
...
def mask_heads(args, model, eval_dataloader):
    ...
    while current_score >= original_score * args.masking_threshold:
        ...
        preds = (
            np.argmax(preds, axis=1) if args.output_mode == "classification" else np.squeeze(preds)
        )
...
def prune_heads(args, model, eval_dataloader, head_mask):
    ...
    heads_to_prune = dict(
        (layer, (1 - head_mask[layer].long()).nonzero().tolist()) for layer in range(len(head_mask))
    )
    ...
    logger.info(
        "Pruning: score with masking: %f score with pruning: %f", score_masking, score_pruning
    )
    logger.info(
